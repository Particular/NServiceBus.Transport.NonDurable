...
public class When_emitting_process_span_in_non_inline_receive_path
{
    ...
    public async Task Should_create_process_span_in_the_non_inline_receive_path()
    {
        ...
        var sendActivity = transportActivities.Single(activity => activity.OperationName == NonDurableTransportTracing.SendActivityName);
        var processActivity = transportActivities.Single(activity => activity.OperationName == NonDurableTransportTracing.ProcessActivityName);
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(processActivity.Events.Any(e => e.Name == "nondurable.handoff"), Is.True);
        }
    }
}
