...
public class When_simulating_receive_delay_with_queue_override
{
    ...
    public async Task Should_use_queue_operation_settings_over_broker_defaults()
    {
        ...
        options.ForQueue("input").Receive.RateLimit = new NonDurableRateLimitOptions { PermitLimit = 2, Window = TimeSpan.FromSeconds(30) };
...
public class When_simulating_receive_delay
{
    ...
    public async Task Should_wait_for_simulated_time_to_advance()
    {
        ...
        await using var broker = new NonDurableBroker(new NonDurableBrokerOptions
        {
            ...
            Receive = { RateLimit = new NonDurableRateLimitOptions { PermitLimit = 1, Window = TimeSpan.FromSeconds(5) } }
        });
