...
public class When_simulating_send_with_rate_limiter_factory
{
    ...
    public async Task Should_create_and_reuse_limiter_per_operation_and_queue()
    {
        ...
        Assert.ThrowsAsync<NonDurableSimulationException>(async () => await NonDurableBrokerSimulationTestHelper.Dispatch(dispatcher, "msg-2", "queue"));

        await NonDurableBrokerSimulationTestHelper.Dispatch(dispatcher, "msg-3", "other-queue");
        Assert.ThrowsAsync<NonDurableSimulationException>(async () => await NonDurableBrokerSimulationTestHelper.Dispatch(dispatcher, "msg-4", "other-queue"));
