...
public class When_queue_level_default_rate_limit_takes_precedence_over_broker_default
{
    ...
    public async Task Should_apply_queue_default_for_send_over_broker_default()
    {
        ...
        options.ForQueue("queue").Default.RateLimit = new NonDurableRateLimitOptions { PermitLimit = 2, Window = TimeSpan.FromSeconds(30) };
...
public class When_queue_operation_override_takes_precedence_over_queue_default
{
    ...
    public async Task Should_apply_send_operation_override_over_queue_default()
    {
        ...
        // Queue-level default
        options.ForQueue("queue").Default.RateLimit = new NonDurableRateLimitOptions { PermitLimit = 5, Window = TimeSpan.FromSeconds(30) };
        // Queue + Send operation override - should take precedence
        options.ForQueue("queue").Send.RateLimit = new NonDurableRateLimitOptions { PermitLimit = 1, Window = TimeSpan.FromSeconds(30) };
...
public class When_operation_level_override_takes_precedence_over_broker_default
{
    ...
    public async Task Should_apply_receive_operation_override_over_broker_default()
    {
        ...
        {
            TimeProvider = simulatedTime,
            Default = { RateLimit = new NonDurableRateLimitOptions { PermitLimit = 5, Window = TimeSpan.FromSeconds(30) } },
            Receive = { RateLimit = new NonDurableRateLimitOptions { PermitLimit = 1, Window = TimeSpan.FromSeconds(30) } }
        };
...
public class When_mode_precedence_is_respected_for_send_configuration
{
    ...
    public void Should_apply_queue_operation_mode_over_broker_mode()
    {
        var options = new NonDurableBrokerOptions
        {
            Send = { Mode = NonDurableSimulationMode.Delay }
        };
        // Queue + operation override should take precedence
        options.ForQueue("queue").Send.Mode = NonDurableSimulationMode.Reject;

        Assert.DoesNotThrow(() => new NonDurableBroker(options));
    }
}
...
public class When_mode_precedence_is_respected_for_send_behavior
{
    ...
    public async Task Should_use_queue_operation_mode_reject_for_send()
    {
        ...
        await Dispatch(dispatcher, "msg-1", "queue");
        Assert.ThrowsAsync<NonDurableSimulationException>(async () => await Dispatch(dispatcher, "msg-2", "queue"));
    }
}
...
public class When_mode_precedence_is_respected_for_receive
{
    ...
    public async Task Should_use_queue_operation_mode_reject_for_receive()
    {
        var options = new NonDurableBrokerOptions
        {
            Receive = { Mode = NonDurableSimulationMode.Delay }
        };
        options.ForQueue("input").Receive.Mode = NonDurableSimulationMode.Reject;
        options.ForQueue("input").Receive.RateLimit = new NonDurableRateLimitOptions { PermitLimit = 1, Window = TimeSpan.FromSeconds(30) };
...
public class When_mode_precedence_is_respected_for_delayed_delivery
{
    ...
    public async Task Should_use_queue_operation_mode_reject_for_delayed_delivery()
    {
        var options = new NonDurableBrokerOptions
        {
            DelayedDelivery = { Mode = NonDurableSimulationMode.Delay }
        };
        options.ForQueue("queue").DelayedDelivery.Mode = NonDurableSimulationMode.Reject;
        options.ForQueue("queue").DelayedDelivery.RateLimit = new NonDurableRateLimitOptions { PermitLimit = 1, Window = TimeSpan.FromSeconds(30) };
...
public class When_full_precedence_chain_is_validated_for_send
{
    ...
    public async Task Should_respect_full_precedence_hierarchy()
    {
        ...
        var options = new NonDurableBrokerOptions
        {
            ...
            Send = { RateLimit = new NonDurableRateLimitOptions { PermitLimit = 10, Window = TimeSpan.FromSeconds(30) } },
            Default = { RateLimit = new NonDurableRateLimitOptions { PermitLimit = 20, Window = TimeSpan.FromSeconds(30) } }
        };
